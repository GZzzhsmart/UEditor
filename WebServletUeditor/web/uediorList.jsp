...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
        ...
        <a class="btn btn-primary radius" data-title="添加资讯"  onclick="article_add('添加资讯','<%=path%>/addUeditor.jsp','800','')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加资讯</a></span>
            ...
            <c:forEach items="${ueditorList}" var="s">
                <tr class="text-c">
                    ...
                    <td class="f-14 td-manage">
                        <a style="text-decoration:none" class="ml-5" onClick="article_edit('资讯编辑','<%=path%>/ueditors?order=find&id=${s.id}','800','')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
...
<script>
    ...
    function listrefush(){
        location.href="<%=path%>/ueditors?order=list";
    }
</script>
<script type="text/javascript">
    ...
    function  del() {
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post(
                '<%=path%>/ueditors?order=deletemanay&id='+a,
                {
                    "id":a
                },
                function(data){
    ...
    function article_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: '<%=path%>/ueditors?order=delete&id='+id,
                dataType: 'text',
